...
<div class="row table-responsive" style="height: 450px;">
    <table class="table table-striped " >
        ...
        <tr>
            <td align="right" >ຈ​ຳ​ນວນ​ເງ​ີນ​ຈ່າຍ:</td>
            <td>
                <script>
                    function calc()  // function calculation price
                    {
                        ...
                        if (v3 > 0)
                        {
                            ...
                            document.getElementById('p_y').innerHTML = num + " ກີບ";
                            document.getElementById('p_d').innerHTML = "0.00" + " ກີບ";
                            ...
                            if (isNaN(v3) == false)
                            {
                                document.getElementById('p_y').innerHTML = "0.00" + " ກີບ";
                                var value1 = v4;
                                var num1 = value1.toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
                                document.getElementById('p_d').innerHTML = num1 + " ກີບ";
        ...
        <tr>
            <td align="right">ຈ​ຳ​ນວນ​​ເງີນຄ້າງ:</td>
            <td style="color: red"><b><div id="p_y"><?= number_format(\Yii::$app->session['paystill'], 2) ?> ກີບ</div></b></td>

        </tr>
        <tr>
            <td align="right">ຈ​ຳ​ນວນ​​ເງີນຖອນ:</td>
            <td style="color: green"><b><div id="p_d"><?= number_format(\Yii::$app->session['paychange'], 2) ?> ກີບ</div></b></td>
    ...
    <div id="sh_l" class="col-md-12" align="right" style="display: <?= (isset(Yii::$app->session['paystill']) && Yii::$app->session['paystill'] == 0) ? "none" : "block" ?>">
        <a href="#" class="btn btn-large bg-yellow"><span class="glyphicon glyphicon-alert"></span> ຢັ້ງ​ຢື້ນ​ຈ່າຍ​ເງີນ</a>
    </div>
</div>
<?php
...
<div class="row" style="border-top: 2px green solid; padding-top: 2px;">
    <div class="col-md-6">
        <?php
        echo yii\helpers\Html::a('<span class="glyphicon glyphicon-backward"></span> ກັບ​ຄ​ືນ', '#', [
            'onclick' => "
                        $.ajax({
                       type     :'POST',
                       cache    : false,
