...
$mver="2012041901";		// version, remove or comment to avoid upgrades
...
foreach($buf as $nf =>$b) {
  ...
  if ($b['p']) { // {{{ es parseable
	...
	sprintf(__("%d lineas de codigo de depuracion eliminadas por mkInstaller"),$lcem) :
...
if ($tlcem)
  logea(sprintf(__("%d lineas de codigo de depuracion eliminadas por mkInstaller"),$tlcem));
if ($tcem)
...
if ($defbdds and $myopt['ski'])
  $phpg.=
'	    echo "  <tr><td id=tdskdb class=lab>".htmlentities(st("Igdb"),ENT_COMPAT,"ISO-8859-1")." <td id=tiskdb class=tin><input id=inskdb name=\"skdb\" type=checkbox class=che>\n";
';
if ($myopt['nru'])
  $phpg.=
'	    echo "  <tr><td id=tdnrun class=lab>".htmlentities(st("Nesc"),ENT_COMPAT,"ISO-8859-1")." <td id=tinrun class=tin><input id=innrun name=\"nrun\" type=checkbox class=che>\n";
';
if ($fsimu=$myopt['fsm']) {
  unset($myopt['sim']);
  $fsimu='='.$fsimu;
...
if ($defbdds) {
  ...
  foreach($defbdds as $idb => $bd) {
	...
	$_qu=mysql_query; $_err=mysql_error; $_nr=mysql_num_rows; $_fr=mysql_fetch_row; $_es=mysql_real_escape_string; $_fe=mysql_free_result; $_se='`'; $emxrw=$mxrw;
      }
      else {
...
function mmIniStr($my) {
  ...
  $lang_lang=array(
    ...
    '%d lineas de codigo de depuracion eliminadas por mkInstaller' => array(
      'ca' => '%d llinies de codi de depuracio eliminades per mkInstaller',
